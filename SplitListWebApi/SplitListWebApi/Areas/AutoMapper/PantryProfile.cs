        ...
        public PantryProfile()
        {
            ...
            CreateMap<PantryModel, PantryDTO>().PreserveReferences()
                .ForMember(dto => dto.Items, opt => 
                    opt.MapFrom(pm => pm.PantryItems.Select(im => im.ItemModel.Name).ToList()))
                .ForMember(dto => dto.Group, opt =>
                        opt.MapFrom(pm => pm.GroupModel))
                .ForMember(dto => dto.GroupID,
                    opt => opt.MapFrom(pm => pm.GroupModelModelID));
        }
    }
}
